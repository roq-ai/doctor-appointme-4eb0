...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getInsurancePlans() {
    ...
    const response = await prisma.insurance_plan
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'insurance_plan'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createInsurancePlan() {
    await insurancePlanValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.insurance_plan.create({
      data: body,
    });
